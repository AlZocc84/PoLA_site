...
def prepare_test_data(dataframe, normalize=False, return_index=False, return_normalization_col=True):
    ...
    n, Ycol = Y.shape
    print(f'loaded file : found {n} instances of {Ycol} dimensions')

    Y = Y.values
    total_vol = total_vol.values

    if normalize:
        Y = Y/total_vol[:, np.newaxis]
