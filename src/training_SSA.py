...
from data import load_both_data_SSA
...
def parse_args():
    ...
    parser.add_argument(
        "--save",
        type=str,
        default="./neuralregressorSSA",
        help="Directory where to store the learned models (default: ./neuralregressorSSA)."
    )
...
def NewPAEzy(model_path=None, cuda=False):
    return NN3WayReg(
        model=AE(
            Xdim=10, Zdim=10, Ydim=1, Ldim=1024, hidden=[1024]
        ), wX=0, wZ=0.0001, X_red=10, Z_red=10, Y_red=1, lr=0.001, cuda=cuda,
...
def NewPAE2zy(model_path=None, cuda=False):
    return NN3WayReg(
        model=AE2(
            Xdim=10, Zdim=10, Ydim=1, Ldim=1024, hidden=[1024]
        ), wX=0, wZ=0.001, X_red=10, Z_red=10, Y_red=1, lr=0.001, cuda=cuda,
        smooth_prediction=False, smooth_reg_weight=0.000, weight_decay=0.00000001, max_epochs=25_000, normalize_XY=True,
...
if __name__ == '__main__':
    ...
    path_SSA = args.SSA_output
    path_n2 = args.gas_input

    cuda = not args.cpu
    print(f'Input gas: {path_n2}')
    print(f'Output SSA: {path_SSA}')
    print(f'Save directoty: {model_path}')
    print(f'Exclude models: {args.exclude_models}')
    print(f'Cuda {cuda}')

    test_names, Vin, Gin, Gout = load_both_data_SSA(
        path_input_gas=path_n2, path_output_gas=path_SSA, cumulate_vol=True, normalize=True,
